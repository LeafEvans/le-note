...
<img src="../../images/go-programming/image_20260403_161820.webp" style="zoom:80%;" />
...
<img src="../../images/go-programming/image_20260404_054902.webp" style="zoom:67%;" />

#### `with`

Go 模板内置动作，**用于临时切换上下文**，将代码块内的 `.` 指向指定对象，简化嵌套数据的访问。
...
> [!tip]
>
> 1. 引入模版时，必须使用子模板内 `define` 定义的名称（如 `page_header.html`）；
> 1. 引入模板需在末尾传入 `.`，表示将**当前上下文数据**传递给子模板使用。
...
> [!tip]
>
> `c.ShouldBind()` 方法默认仅支持解析并绑定 URL 查询参数、表单数据与 JSON 数据。
...
中间件通过 `gin.Context` 的 `Set` 存数据，控制器用 `Get` 取数据，实现单次请求内的数据共享。
...
应用简单时，可直接在 Controller 中处理业务逻辑；若有需在多控制器 / 模板复用的功能，需将公共功能抽离为 Model 模块。

Model 是业务逻辑的逐步抽象，通常封装公共方法供多 Controller 调用，也可实现数据库交互。
     ...
     <body>
       ...
       >
         用户名：<input
           ...
           placeholder="用户名"
         /><br /><br />
         头 像 1：<input type="file" name="avatar1" /><br /><br />
         头 像 2：<input type="file" name="avatar2" /><br /><br />
         <input type="submit" value="提交" />
       </form>
     </body>
   </html>
...
- 立即删除 Cookie：设为 `-1`（负数）
- 会话级 Cookie（关闭浏览器失效）：设为 `0`
- 持久化 Cookie（指定有效期）：设为大于 `0` 的秒数
...
1. 将子域名 `a.leafevans.com` 和 `b.leafevans.com` 解析至同一台服务器。

2. 在 `a.leafevans.com` 中配置 Cookie 时，将 `domain` 设为主域名，即可让 `b.leafevans.com` 等子域名共享该 Cookie，示例代码：

   ```go
   // 现代浏览器推荐写法：直接使用纯主域名，无需加 .
   c.SetCookie("username", "曼波", 3600, "/", "leafevans.com", false, true)
   // 旧版兼容写法（已无必要）：.leafevans.com
   // c.SetCookie("username", "曼波", 3600, "/", ".leafevans.com", false, true)
   ```

> [!tip]
>
> 1. 现代浏览器中，`domain` 设为 `leafevans.com`（纯主域名）与 `.leafevans.com`（以点开头）效果一致，均表示该 Cookie 对 `leafevans.com` 及其所有子域名（如 `www.leafevans.com`、`api.leafevans.com`）生效；
> 2. 以点开头是早期 IE 浏览器的兼容写法，目前主流浏览器（Chrome/Firefox/Edge）已无需添加，推荐使用纯主域名配置。
