...
**切片（Slice）**是一个拥有相同类型元素的<u>*可变长度*</u>的序列。它是基于数组类型做的一层封装。它非常灵活，支持自动扩容。
...
> [!note]
>
> 零值为 `nil` 的类型本质上是引用类型，`nil` 表示它们未指向任何实际数据。
...
> [!note]
>
> 上述方式称为**切片表达式**，是一种浅拷贝操作，会创建一个新的切片头并指向原有数组或切片的底层数据。在切片未触发扩容时，对新切片或原数据的修改会同步影响另一方；一旦切片因 `append` 等操作触发扩容（Go 会新建更大的底层数组），新切片便与原数据解除关联，此时修改不再互相影响。
