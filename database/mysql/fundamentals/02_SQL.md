...
> - 各字符串 / 二进制类型的最大存储长度，均匹配「对应位数无符号数的最大值」，规律如下：
>   - `TINYTEXT` / `TINYBLOB` → 占用 8 位（1 字节），最大值为 2^8^ - 1 = **255** 字节。
>   - `TEXT` / `BLOB` → 占用 16 位（2 字节），最大值为 2^16^ - 1 = **65535** 字节。
>   - `MEDIUMTEXT` / `MEDIUMBLOB` → 占用 24 位（3 字节），最大值为 2^24^ - 1 = **16777215** 字节。
>   - `LONGBLOB` / `LONGTEXT` → 占用 32 位（4 字节），最大值为 2^32^ - 1 = **4294967295** 字节。
