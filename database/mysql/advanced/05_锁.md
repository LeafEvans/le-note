...
锁是协调多进程 / 线程并发访问控制**共享资源**的**核心机制**。

在数据库中，数据作为多用户共享的**关键资源**，其并发访问的**一致性与有效性**是系统必须解决的**核心问题**；**锁冲突**则直接影响数据库**并发性能**。

因此，数据库锁不仅**至关重要**，其机制也更为复杂。
...
1. **记录锁（Record Lock）**：锁定单个索引记录的锁，核心作用是阻止其他事务对该行执行 `UPDATE`、`DELETE` 等写操作。在 **RC** 和 **RR** 隔离级别下均支持。
...
1. 当基于唯一索引执行等值检索，且匹配到已存在的记录时，临键锁会自动优化为**记录锁（Record Lock）**。
...
2. InnoDB 会对表中**每一行都加行锁**，这种“全表记录锁”的效果等价于表级锁。
3. 最终会阻塞其他事务对表中任何行的读写操作，这一现象称为「记录锁的表级锁等效升级」。

> [!tip]
>
> **RC 级别核心操作机制**：
>
> - RC 普通 SELECT：采用**语句级 Read View**，每执行一次新建一个，无法实现可重复读。
> - RC 下 DML（UPDATE/DELETE）：跳过 Read View，通过**半一致性读**读取行的**最新已提交版本**匹配条件；不匹配立即释放锁，匹配则持锁至事务结束，提升并发效率。
>
> **一致性读与半一致性读**：
>
> - **一致性读**：RC/RR 下普通 SELECT 专用，基于 Read View 实现无锁快照读，保障读一致性。
> - **半一致性读**：仅 RC 下 DML 专用，读取行最新提交版本，核心作用是快速判断锁的释放时机。
