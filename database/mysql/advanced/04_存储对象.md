...
```mariadb
-- 查询指定数据库的存储过程及状态信息
SELECT * FROM information_schema.ROUTINES WHERE ROUTINE_SCHSEMA = 'xxx';
-- 查询某个存储过程的定义
...
> [!note]
>
> `CHARACTER SET` 是 MySQL 中**指定字符集**的关键字，用于定义数据库、表或列的字符编码规则，可简写为 `CHARSET`。
...
DECLARE cur_users CURSOR FOR SELECT name, profession FROM user WHERE age <= p_max_age;
DECLARE EXIT HANDLER FOR NOT FOUND CLOSE cur_users;
