...
- **行**：InnoDB 数据的存储单位，包含两个关键隐藏列：
  - `DB_TRX_ID`：记录每次被改动时，对应的事务 ID。
  - `DB_ROLL_PTR`：记录每次被改动时，旧版本数据写入 undo 日志的指针，可通过它追溯记录修改前的信息。
...
- **Log Buffer（日志缓冲区）**：InnoDB 引擎专为 Redo Log 设计的专属内存缓冲区，用于缓存待刷盘的 Redo Log 记录（包含 Undo 数据页修改对应的 Redo Log），默认大小为 16MB。缓冲区中的日志会按配置规则定期刷新到磁盘，在大事务、批量数据修改的场景下，适当调大缓冲区大小可有效减少磁盘 IO 开销。
...
- **Undo Tablespaces（撤销表空间）**：MySQL 实例初始化会自动生成两个默认的撤销表空间（初始大小为 16MB），用于存储支撑事务回滚的 undo log 日志。
  ...
  <img src="../../../images/database/image_20260331_100341.webp" style="zoom:50%;" />

  > [!note]
  >
  > MySQL 8.0 默认生成两个独立表空间（`undo_001`/`undo_002`），以便在文件间切换执行动态清理。
...
- **Doublewrite Buffer Files（双写缓冲区文件）**：InnoDB 引擎在将 Buffer Pool 中的数据页刷新到磁盘前，会先将数据页写入双写缓冲区文件，用于在系统异常时实现数据恢复。
  ...
  <img src="../../../images/database/image_20260331_110605.webp" style="zoom: 50%;" />
...
- **可重复读（Repeatable Read）**：事务内的第一个 `SELECT` 语句会生成快照，后续查询将复用该快照。
...
**回滚日志（undo log）**是 InnoDB 存储引擎在执行 `INSERT`、`UPDATE`、`DELETE` 等写操作时生成的日志，核心作用是支持事务回滚，同时为 MVCC（多版本并发控制）提供历史数据版本。
...
<img src="../../../images/database/image-20260401-182600.svg" style="zoom:80%;" />
