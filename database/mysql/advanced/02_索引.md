...
- 若存在主键，主键索引即聚集索引。
- 若表未定义主键，InnoDB 会选择第一个**非空的唯一（`UNIQUE`）索引**作为聚集索引。
- 若表无主键，也没有合适的唯一索引，则 InnoDB 会自动生成一个 `DB_ROW_ID` 作为隐藏的聚集索引。
...
MySQL 的慢查询日志默认未开启，需在 MySQL 配置文件（Linux 系统常见路径为 `/etc/mysql/mysql.conf.d/mysqld.cnf`，不同系统路径可能略有差异）中配置如下信息：
...
> [!note]
>
> `mysqld.cnf` 是 MySQL 服务端（`mysqld` 进程）的核心配置文件，`mysql.cnf` 是 MySQL 客户端程序的配置文件。
>
> <img src="../../../images/database/image_20260309_101442.png" style="zoom:67%;" />

配置完毕后，通过以下指令重启 MySQL 服务器进行测试，查看慢日志文件中记录的信息 `/var/lib/mysql/[主机名]-slow.log`：
...
可以通过配置 `/etc/mysql/mysql.conf.d/mysqld.cnf` 中的 `slow_query_log_file` 参数，自定义慢查询日志的存储路径与文件名。

<img src="../../../images/database/image_20260309_103108.png" style="zoom: 50%;" />
...
前后均带通配符的模糊匹配，和头部模糊匹配效果一致，索引同样失效，触发全表扫描。
...
从执行计划可见，该查询仅命中了 `uniq_user_name` 这个单列索引。由于 `phone` 字段不在此索引的覆盖范围内，因此必须通过回表查询才能获取完整数据，这正是覆盖查询可以解决的问题。
