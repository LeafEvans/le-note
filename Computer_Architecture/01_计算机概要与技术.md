...
使用 100 Mbps 来表示百兆；转换成字节表示，即为 12.5MB/s。
...
3. 计算机设计的八个伟大思想是？
4. Amdahl 定律的公式为？$t_{\text{改进}} = \frac{t_\text{相关}}{S} + t_{\text{不相关}}$
...
所以，每条机器指令占用的时钟周期数（cycles per instruction）是一个平均值，即 $\dfrac{\text{总周期数}}{\text{总指令数}}$。
