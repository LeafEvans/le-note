...
其中，为了便于[nu]+[G]快捷键的使用，使用命令`:set nu`来显示每一行的行号，关闭这个需要输入命令:<br />`set nonu`。
<a name="Xhj9x"></a>
