...
3. 因为内联函数在最终生成的代码中是没有定义的，所以内联函数的作用域可以理解为只在定义的文件内。假如在`a.cpp`中定义了`inline int func(int a) { return ++; }`，如果在`b.cpp`中需要调用则在`b.c`中重新定义内联函数`inline int func(int a) { return ++; }`
