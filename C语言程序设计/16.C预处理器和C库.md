...
在预处理阶段，会把头文件内的内容包含到test.i中（#include的这种指令），并且进行了MAX的替换和注释的删除。
...
如g_val，main，Add（局部不用）。

- 汇编阶段：

生成test.o，把汇编代码转换为二进制代码，形成符号表（右边即是符号表）。
...
在Linux环境下，它以elf格式来组织文件。

将add.o、test.o、链接库合并成一个程序。

进行合并的时候，对上图Add的地址进行了筛选，可知上面那个0x100才是有效的地址。
