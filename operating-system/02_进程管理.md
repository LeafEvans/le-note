...
<img src="https://leafalice-image.oss-cn-hangzhou.aliyuncs.com/img/image-20250602210822551.png" alt="image-20250602210822551" style="zoom:33%;" />
...
- <span style="color:#725CAD">**互斤关系**</span>（`mutex = 1`）：对缓冲区（盘子）的访问要互序地进行
...
<span style="color:#1DCD9F">原因在于对 `count` 变量的检查和赋值无法一气呵成</span>，因此可以设置另一个互斥信号量来保证各读进程对 `count` 的访问是互斥的。
