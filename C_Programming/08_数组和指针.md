...
在 C99 标准之前，数组的大小必须是常量或者常量表达式；

在 C99 标准之后，数组的大小可以是变量，为了支持变长数组。在支持 C99 标准的编译器上可以运行，而 MSVC 不支持，而 gcc 支持。同时，变长数组不支持初始化。
...
内存被切割成一个个内存单元，一个内存单元是 1 byte（1 个字节）。
...
而上面的组合有$2^{32}$种，有$2^{32}$个地址。64 位机器同理。

在 32 位机器上，因为有 32 位数，所以要 4 个字节来存储。
...
由上述代码可知，**指针的类型决定了指针 $+1$ 或 $-1$ 时（例子是 1，可以是整数），跳过字节的个数**，如 `int` 类型是跳过 4 个字节，`char` 类型是跳过 1 个字节。
...
由此可知，此时 `p != NULL` 这句代码不能实现，并且虽然变量 `a` 已经销毁，但在那个地址的值还没有被其他值覆盖，所以解引用的时候依旧会输出 10。
